...
inline void smitetarget(IUnit* target)
{
	if (smite != nullptr)
	{
		if (!usesmitetarget->Enabled() || !smite->IsReady()) return;
		if (target != nullptr && myHero->IsValidTarget(target, 550))
		{
			auto Dmg = GDamage->GetSummonerSpellDamage(myHero, target, kSummonerSpellSmite);
